...
For each `BoundaryDefinitionAction` there is one and only one `/Core/Relationship` class or subclass with the relationshipType of `creatorBy` on the to and a `/Core/Agent` class or subclass on the from.
